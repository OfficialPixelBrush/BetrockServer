...
void SendToPlayer(std::vector<uint8_t> &response, Player* player) {
	if (player && player->connectionStatus > Disconnected && response.size() > 0) {
		if (debugSentPacketType) {
			std::cout << "Sending " << PacketIdToLabel(response[0]) << " to " << player->username << "! (" << response.size() << " Bytes)" << std::endl;
		}
