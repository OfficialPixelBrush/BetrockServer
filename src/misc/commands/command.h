...
#define DEFINE_COMMAND(name, label, description, syntax, requiresOp, requiresCreative)                                 \
	class name : public Command {                                                                                      \
	  public:                                                                                                          \
		...
		std::string Execute(std::vector<std::string> command, std::vector<uint8_t> &response,                          \
...
class Command {
  ...
  public:
	...
	virtual std::string Execute(std::vector<std::string> parameters, std::vector<uint8_t> &response,
								Client *client) = 0;
