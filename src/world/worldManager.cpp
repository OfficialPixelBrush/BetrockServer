...
QueueChunk::QueueChunk(Int2 pPosition, const std::shared_ptr<Client> &pRequestClient) {
...
void WorldManager::WorkerThread() {
	...
	while (Betrock::Server::Instance().IsAlive()) {
		...
		std::shared_ptr<Chunk> c = GetChunk(cq.position, generator.get());
...
std::shared_ptr<Chunk> WorldManager::GetChunk(Int2 position, Generator *generator) {
	// If the chunk already exists, return right away
	std::shared_ptr<Chunk> c = world.GetChunk(position);
	if (c)
		return c;
	...
	// if (c) return c;
	// Old .ncnk format (NBT)
	if (world.ChunkFileExists(position)) {
		c = world.LoadOldV2Chunk(position);
		c->state = ChunkState::Populated;
		return c;
	}
	// Old .cnk format (Raw)
	if (world.ChunkFileExists(position, OLD_CHUNK_FILE_EXTENSION)) {
		c = world.LoadOldChunk(position);
		c->state = ChunkState::Populated;
		return c;
	}
	...
	if (world.ChunkExists(position + Int2{1, 1}) && world.ChunkExists(position + Int2{0, 1}) && world.ChunkExists(position + Int2{1, 0})) {
		tryPopulate(position);
	}
	if (world.ChunkExists(position + Int2{-1, 1}) && world.ChunkExists(position + Int2{0, 1}) && world.ChunkExists(position + Int2{-1, 0})) {
		tryPopulate(position + Int2{-1, 0});
	}
	if (world.ChunkExists(position + Int2{1, -1}) && world.ChunkExists(position + Int2{0, -1}) && world.ChunkExists(position + Int2{1, 0})) {
		tryPopulate(position + Int2{0, -1});
	}
	if (world.ChunkExists(position + Int2{-1, -1}) && world.ChunkExists(position + Int2{0, -1}) && world.ChunkExists(position + Int2{-1, 0})) {
		tryPopulate(position + Int2{-1, -1});
	}
